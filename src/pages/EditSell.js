...
const EditSell = () => {
    ...
    const fetchListing = async () => {
        try {
            ...
            const response = await fetch(`${BaseUrl}/listing/edit/${id}`);
            const result = await response.json();

            if (!response.ok) {
                console.error('API Error:', result);
                alert(`Error: ${result.message || 'Failed to fetch listing'}`);
                return;
            }

            // setFormData(result);
            console.log('Result:', result.accountType, result.platform);
            ...
            setFormData({
                ...
                accountPrice: result.accountPrice || '',
                ...
                exchangeRequirements: result.exchangeRequirements || '',
                preferredPayment: result.preferredPayment || 'paypal',
                termsAgree: result.termsAgree || false
            });

        } catch (error) {
            console.error('Error fetching listing:', error);
            alert('There was a problem fetching the listing. Please try again.');
        }
    }
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const listingData = {
                ...
                uploadImage: cloudinaryData?.secure_url || formData.uploadImage,
